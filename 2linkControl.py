...
meanX = np.array([1.060287521, 1.060287521, -30, -30, 0.044834633, -0.026438883, 0])
meanY = np.array([1.030287521, 1.030287521, -11.29412641, -91.16077679])
stdX = np.array([0.676768727, 0.676768727, 172.3377411, 172.3377411, 155.8123305, 155.5320226, 1])
...
def main():
    ...
    saver = tf.train.import_meta_graph('./modelANN/ANN.meta', clear_devices=True)
    saver.restore(sess, './modelANN/ANN')
    ...
    targetTheta1 = 1.0*70*np.pi/180
    targetTheta2 = 1.0*10*np.pi/180
    ...
    for k in range(0, period):
        ...
        for j in range(0, step):
            ...
            Input = [[theta1], [theta2], [omega1], [omega2], [u1], [u2], [1.0]]
            Input_arr = (np.array(Input).reshape(7)-meanX)*(1/stdX)
        ...
        CX[:, k+1] = ArmDynamic(CX[:, k], U, target*stdX[0:4]+meanX[0:4])
        CU[0][k] = U[0]
        CU[1][k] = U[1]
    ...
    plt.plot(x, x-(x-60), label = "kijyunn", color = "green")
    plt.plot(x, x-(x-20), label = "kijyunn", color = "orange")
